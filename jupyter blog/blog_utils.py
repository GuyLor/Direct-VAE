...
def h_i_and_h_ij_grads(h_i,h_ij,f_theta,z,z_opt,N,epsilon=1.0,solver= 'maxflow'):
    K=h_i.size(-1)
    ...
    if solver == 'maxflow':
        k = argmax_maxflow(batch_h)
    elif solver=='cplex':
        k = argmax_cplex(batch_h)
    elif solver =='none':
        k = h_i.argmax(-1)
    z_direct = k.tolist()
    z_opt =  z_opt.tolist()

    z_direct_1hot = torch.zeros_like(h_i).scatter_(-1, k.view(-1, 1),1.0)
    gradients_sign_h_i = (z - z_direct_1hot).view(-1,N,K) 
